...
void _swapDynArr(DynArr *v, int i, int  j) {
	TYPE  temp;
	assert(v!=0);
	assert(i < v->size);
	assert(j < v->size);
...
void addToBag(struct bag* b, TYPE val) {addDynArr(b->dynArr, val);}
void removeFromBag(struct bag* b, TYPE val) {removeDynArr(b->dynArr, val);}
int containsBag(struct bag* b, TYPE val) {return(containsDynArr(b->dynArr, val));}
int isEmptyBag(struct bag* b) {return(isEmptyDynArr(b->dynArr));}
void printBag(struct bag *b) {_printDynArr(b->dynArr);}
...
int _indexSmallest(DynArr *heap, int i, int j);
void _adjustHeap(DynArr *heap, int max, int pos);
void _swap (DynArr *v, int i, int j);
...
void addHeap(DynArr *heap, TYPE val) {
     int pos = sizeDynArr(heap);   // idx of first empty spot
     int parIdx = (pos - 1)/2; 
     TYPE parVal;
     
     // Check if array needs resizing (if pos > capacity)
     if (pos >= heap->capacity)
          _dynArrSetCapacity(heap, 2 * heap->capacity); 
     // If pos == 0, we're adding the first element, so no need to compare
     if (pos > 0) 
     ...
     while (pos > 0 && LT(val, parVal)) { 
          ...
          pos = parIdx; 
          parIdx  = (pos - 1) / 2; 
          if (pos > 0) // Get parent (if not at root) 
               parVal = heap->data[parIdx]; 
     }
     // reached root or parent is smaller than new value. 
...
/*	Adjust heap to maintain heap property
	...
	pre:	max <= size
     post:	heap property is maintained for nodes from index pos 
     to index max-1  (ie. up to, but not including max) */
void _adjustHeap(DynArr *heap, int maxIdx, int pos) {
     int leftIdx = 2*pos + 1;  
     int rightIdx = 2*pos + 2;  
     int smallIdx; 
     // Make subtree rooted at pos into a heap
     
     // If pos has two children  
     if (rightIdx < maxIdx){ 
          // get smallest child
          smallIdx = _indexSmallest(heap, leftIdx, rightIdx);  
          // if pos is greater than smallest child, swap
          if( compare(heap->data[smallIdx], heap->data[pos]) == -1 ) {               
                    _swapDynArr(heap, pos, smallIdx);  
               _adjustHeap(heap, maxIdx, smallIdx); // if smallest child has children, continue 
          } 
     } 
     // If pos has one child
     else if (leftIdx < maxIdx) 
          // swap with smallest child, call adjustHeap
          if( compare(heap->data[leftIdx], heap->data[pos]) == -1 ) {               
               _swapDynArr(heap, pos, leftIdx);  
...
void sortHeap(DynArr *heap) {
     ...
     for (i = sizeDynArr(heap)-1; i > 0; i--) { 
          _swapDynArr(heap,i,0);/*Swap last el. with the ﬁrst*/  
          _adjustHeap(heap,i,0); /* build heap property*/	
     } 
}
